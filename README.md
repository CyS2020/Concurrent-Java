...
方法二：run()整个都被重写
- 5. 然后具体展开说其他方式；
还有其他的实现线程的方法，例如线程池、FutureTask(callable)等，它们也能新建线程，但是细看源码，从没有逃出过
...
- 检测与恢复策略：一段时间检测是否有死锁，如果有就剥夺某个资源打开死锁
- 鸵鸟策略：装作看不到
#### 实际工程避免死锁
- 设置超时时间(Lock的tryLock(long timeout, TimeUnit unit)
- 多使用并发类而不是自己设计锁，使用并发集合(比同步集合性能好)
- 尽量降低锁的使用粒度：用不同锁而不是一个锁
- 如果能使用同步代码块，就不使用同步方法：指定锁对象
...
- 死锁是最常见的活跃性问题
- 活锁(LiveLock)
  - 定义：虽然线程并没有阻塞，也始终在运行，但是程序却得不到进展，因为线程始终做同样的事情
  - 引入随机因素解决活锁问题、以太网退避算法
  - 消息队列活锁：消息处理失败则放在对头重试->放在队尾、重试限制、持久化定时重试
- 饥饿
...
6. 如果一个线程处于了阻塞状态(如线程调用了sleep()、join()、wait()、以及可中断的通道上的 I/O 操作方法后可进入阻塞状态)，则在**监控线程**在检查中断标
示时如果发现中断标示为true，则会在这些阻塞方法(sleep()、join()、wait()及可中断的通道上的 I/O 操作方法)调用处抛出InterruptedException异常，
并且在经过异常处理后会立即将线程的中断标示位清除，即重新设置为false。 抛出异常是为了线程从阻塞状态醒过来，并在结束线程前让程序员有足够的时间来处理中断请求。
7. 若想要中断一个上述线程那么其实肯定获取到了该线程的引用，调用Thread.interrupt(); 进行中断操作即可唤醒上述线程；线程运行完成后中断标志位会恢复false
8. 绝不应该在代码中忽略InterruptedException 在捕获异常后会清除中断状态，应该重新抛出中断异常，或者应该在catch中通过Thread.currentThread().interrupt()重新中断线程
9. 可以通过while(!Thread.currentThread().isInterrupted())还可以通过if代替while结合throw new InterruptedException()加catch的形式跳出执行过程，在catch处理必要的资源关闭操作等
10. synchronized与Lock区别：1.底层实现；2.是否可以手动释放；3.是否可中断；4.是否公平；5.是否可以绑定Condition；6.锁的对象; https://zhuanlan.zhihu.com/p/126085068
11. 若不同的对象中包含相同引用，那么并发操作这些不同的对象也会造成线程安全问题；常见的是容器中的对象--虽然objs与origin不同但内部元素却相同，修改内部元素时很容易造成线程安全问题
```
